...
class Detector:
    def __init__(self, detection_size=(160, 160)):
        self.model = SlimNet()
        self.model.load_state_dict(torch.load(open("weight/weights.pth", "rb"), map_location="cpu"))
    ...
    def detect(self, img, bbox):
        ...
        crop_image = torch.tensor(crop_image).float().cpu()
        with torch.no_grad():
            start = time.time()
        ...
        # cl = landmark.copy()
        # cl[:, 0] = landmark[:, 0] * detail[1] + detail[3]
        # cl[:, 1] = landmark[:, 1] * detail[0] + detail[2]
        # for p in cl:
