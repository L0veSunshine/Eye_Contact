...
def get_box(img, shape, base, h, w):
    ...
    b = np.degrees(np.arccos(abs(shape[base][0] - shape[base + 3][0]) / width))
    if shape[base][1] > shape[base + 3][1]:
        b = -b
    ...
    rotate = cv2.getRotationMatrix2D((int(center[0]), int(center[1])), b, 1)
...
if __name__ == '__main__':
    img = cv2.imread("image_0073_t.png")
    f = open("image_0073_t.pts")
    ptsdata = f.readlines()[3:-1]
    tmp = []
    for _one_p in ptsdata:
        xy = _one_p.rstrip().split(' ')
        tmp.append([float(xy[0]), float(xy[1])])
    shape = np.array(tmp).astype(np.int32).reshape((-1, 2))
    # for point in shape:
    #     img = cv2.circle(img, tuple(point), 2, (255, 0, 0), -1, 1)
    #
    # cv2.imshow("1", img)
    # cv2.waitKey(0)
    ll = get_box(img, shape, 36, 60, 100)
    rr = get_box(img, shape, 42, 60, 100)
    cv2.imwrite("ll.jpg", ll)
    cv2.imwrite("rr.jpg", rr)

    # cv2.imwrite("12as.jpg", img)
